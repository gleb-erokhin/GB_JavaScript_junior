...
```javascript
// объявление переменной должно быть выше присваения, либо сразу
...
### Выражение
```javascript
document.write(`выводим текст ${'переменная'}`); - ${} - можно использовать для арифметических выражений и переменных
```
